...
These are the instructions for setting up a development environment for 02Engine itself. This is useful if you want to submit pull requests to us or make your own mod.
...
02Engine is a large app that can require multiple gigabytes of disk space and memory to build.
...
Scratch is broken up into a bunch of different packages, each implementing one part of the app.
 ...
 - **scratch-svg-renderer** helps fix various SVG rendering problems (we rename this to **@02Engine/scratch-svg-renderer**)
 ...
 - **scratch-storage** is an abstraction around fetch() used for downloading (and theoretically uploading) files (we rename this to **@02Engine/scratch-storage**)
 - **scratch-l10n** contains some translations (we rename this to **@02Engine/scratch-l10n**)
...
When deploying 02Engine to a live website, you should enable production mode. This will result in faster execution and a greatly reduced file size:
...
By default 02Engine generates links like `https://02engine.org/editor.html#123`. However, by setting the variables `ROOT=/` and `ROUTING_STYLE=wildcard` (in the same way that you set `NODE_ENV=production`), you can get routes like `https://02engine.org/123/editor` instead. Note that this requires a server that will setup the proper aliases. The webpack development server in scratch-gui is setup for this. For production you'd want something more like https://github.com/02Engine/02engine.org.
...
cd scratch-gui/..
git clone https://github.com/02Engine/scratch-vm
